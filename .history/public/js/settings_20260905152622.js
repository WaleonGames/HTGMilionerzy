...
function createPrizeLevels(prizes){
  ...
  const list=Array.isArray(prizes)&&prizes.length
    ? prizes
  ...
  list.forEach((prize,index)=>{
    ...
    row.className="prize-level";

    const number=document.createElement("div");
    number.className="prize-level-number";
    ...
    guaranteedLabel.className="guaranteed-row";
...
function collectSettings(){
  ...
  return{
    ...
    prizeTree:{
      currency:currency.value.trim()||"zł",
...
function applySettings(settings){
  ...
  const prizeTree=settings.prizeTree||{};
