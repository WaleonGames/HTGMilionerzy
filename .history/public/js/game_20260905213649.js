...
function renderPrizeTree(){
  ...
  if(!levels.length){
    ...
    empty.textContent="Brak poziomów nagród.";

    gamePrizeTree.appendChild(empty);
  ...
  levels
    ...
    .forEach(prize=>{
      ...
      item.dataset.level=String(levelNumber);

      if(prize.guaranteed){
        item.classList.add("guaranteed");
      }
      ...
      level.className="game-prize-level";
      ...
      amount.className="game-prize-amount";

      amount.textContent=formatPrize(
        prize.amount,
        currency
      );
      ...
      gamePrizeTree.appendChild(item);
    });
}
...
function updatePrizeTreeActive(){
  ...
  gamePrizeTree
    .querySelectorAll(".game-prize-item")
    .forEach(item=>{
