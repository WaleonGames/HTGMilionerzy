...
const gameEndScene=
  document.getElementById("gameEndScene");
...
function prepareGameQuestions(){
  ...
  for(
    ...
    if(!pool.length){
      console.warn(
        `Brak aktywnego pytania dla poziomu ${level}.`
      );
...
function shuffleQuestionAnswers(question){
  ...
  if(
    ...
    sameCorrectAnswerStreak++;
  }else{
    previousCorrectAnswer=
      result.correctAnswer;

    sameCorrectAnswerStreak=1;
  }
